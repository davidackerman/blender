...
for i, obj in enumerate(all_objects):
    organelle_type = obj.name.split("_")[0]
    ...
    curve_data = bpy.data.curves.new(name=f"{organelle_type}Path_{i}", type='CURVE')
    curve_data.dimensions = '3D'
    spline = curve_data.splines.new('POLY')
    spline.points.add(1)  # 2 points: start and end
    print(obj.name,original_location)
    if organelle_type=="Cell":
        spline.points[0].co = (0,0,0, 1)
        spline.points[1].co = (true_center[0]*2, true_center[1]*2, 0, 1)
    else:
        spline.points[0].co =  (0,0,0, 1)
        spline.points[1].co =   (0.001,0.001,0.001, 1) # if we have the same point it disappears

    # Create an object from the curve data and link it to the scene
    curve_obj = bpy.data.objects.new(f"{organelle_type}PathObj_{i}", curve_data)
...
for obj in ecs_objects:
    ...
    sb_mod.settings.goal_spring     = 0.3   
    sb_mod.settings.goal_default    = 0.4   
    sb_mod.settings.goal_max        = 0.4   
    ...
    # sb_mod.settings.goal_default  = 0.8
    # sb_mod.settings.goal_max      = 0.8
...
output_path = "/Users/ackermand/Documents/programming/blender/cello/output/frames_follow_swap_stiffer_camera_moved/"
